...
# 주제를 LLM 에 보낼 최소 질문 수. 1 이면 전부 보낸다.
#
# 처음에는 4 였다. "우연일 수 있다" 는 이유였는데, 실제로 돌려보니 해가 더 컸다.
#   · 첫날 103건에서 주제 39개가 통째로 막혔다. 관리자는 그게 뭔지 볼 수도 없었다.
#   · '미분석' 숫자가 눌러도 줄지 않아, 끝나지 않는 할 일처럼 보였다.
#   · 잡음('너 몇 살이야')도 표본 부족으로 걸려서, LLM 이 걸러냈다는 기록이 안 남았다.
#
# 무엇이 안내할 가치가 있는지는 **사람이 정하는 것이 이 시스템의 요점이다.**
# 기계가 숫자로 미리 잘라내면 그 판단 기회 자체가 사라진다.
# 빈도는 중간 보고에 그대로 보이므로, 적게 물어본 주제는 관리자가 안 고르면 된다.
MIN_CLUSTER_SIZE = 1
